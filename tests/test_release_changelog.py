...
def test_rejects_version_or_date_mismatch() -> None:
    with pytest.raises(ValueError, match="versionが不一致"):
        CHECK.validate_changelog(changelog(), expected_version="0.1.2")
    with pytest.raises(ValueError, match="release日が不一致"):
        CHECK.validate_changelog(
            changelog(), expected_version="0.1.1", expected_date="2026-08-20"
        )
...
def test_current_release_date_uses_jst(monkeypatch) -> None:
    real_datetime = CHECK.datetime

    class FixedDateTime:
        @staticmethod
        def now(zone):
            assert zone.utcoffset(None) == timedelta(hours=9)
            return real_datetime(2026, 8, 19, 12, 0, tzinfo=zone)

    monkeypatch.setattr(CHECK, "datetime", FixedDateTime)

    assert CHECK.current_release_date() == "2026-08-19"
